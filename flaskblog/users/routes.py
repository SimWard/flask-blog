...
def register():
    if current_user.is_authenticated:
        flash('You are already a registered user', 'info')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Login unsuccessful. Please check yoself', 'danger')
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('An email has been sent to help reset your password.', 'info')
        return redirect(url_for('users.login'))
